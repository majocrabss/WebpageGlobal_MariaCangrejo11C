...
function runCsvTest() {
  ...
  if (csvMode === 'mean') {
    ...
    const bS = summaryStats(benchVals);
    const tS = summaryStats(testVals);
    zStat  = zStatMean(tS.mean, bS.mean, bS.sd, tS.n);
    pValue = calcPValue(zStat, csvTail);
    summaryHtml = buildSummaryHtml(benchGroup, bS, testGroup, tS, 'mean');
    ...
    const bS = proportionStats(benchBin);
    const tS = proportionStats(testBin);
    zStat  = zStatProp(tS.phat, bS.phat, tS.n);
    pValue = calcPValue(zStat, csvTail);
    summaryHtml = buildSummaryHtml(benchGroup, bS, testGroup, tS, 'proportion');
    resultsHtml = buildResultsHtml(zStat, pValue, alpha);
  }
  ...
  const box    = document.getElementById('csvDecisionBox');
  const title  = document.getElementById('csvDecisionTitle');
  const text   = document.getElementById('csvDecisionText');

  if (reject) {
    box.className   = 'decision-box reject';
    title.textContent = '✗  Reject H₀';
    title.style.color = 'var(--reject)';
    text.textContent  = `p-value (${fmtP(pValue)}) < α (${fmt(alpha,2)}). Sufficient evidence to conclude that "${testGroup}" differs significantly from "${benchGroup}".`;
  } else {
    box.className   = 'decision-box fail';
    title.textContent = '✓  Fail to Reject H₀';
    title.style.color = 'var(--accent2)';
    text.textContent  = `p-value (${fmtP(pValue)}) ≥ α (${fmt(alpha,2)}). Not enough evidence to conclude that "${testGroup}" differs significantly from "${benchGroup}".`;
  }
}
...
function buildSummaryHtml(bName, bS, tName, tS, type) {
  if (type === 'mean') {
    ...
    return `
      <div class="summary-group"><div class="summary-group-title">Benchmark: ${bName}</div>${r('n',bS.n)}${r('Mean',bS.mean)}${r('SD',bS.sd)}${r('Median',bS.median)}</div>
    ...
    return `
      <div class="summary-group"><div class="summary-group-title">Benchmark: ${bName}</div>${r('n',bS.n)}${r('Successes',bS.successes)}${r('p̂',fmt(bS.phat,4))}</div>
